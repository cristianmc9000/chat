        ...
        <div class="wrapper wrapper-content animated fadeIn">
            <div class="row">
                    ...
                    <div class="ibox float-e-margins">
						...
						?>
								...
								<div class="social-body">
									<strong>Nombre grupo: </strong> <?= !empty($room["room_description"]) ? $room["room_description"] : "null"; ?>
								<p></p><hr><p></p>
								<div class="file-option">
									<a href="<?= $_CHAT_FILE ?>?room_id=<?= $room["room_id"] ?>"><button class="btn btn-success btn-rounded btn-sm"><i class="fa fa-sign-in"></i> Ingreso Directo</button></a>
							...
							if(mysqli_num_rows($query) < 1){
								echo "Aun no has creado ningun grupo!";
							}
                ...
                <div class="col-lg-12">
                    <div class="ibox float-e-margins">
                        <div class="ibox-title">
                            <h5>Grupos a los que tienes acceso</h5>
                        </div>
                        <div class="ibox-content">
						<?php
						...
						$room_query = mysqli_query($db, "select * from chat_room where room_id = {$mem["room_id"]}") or error("No puedo obtener los datos", $_HOME_FILE);
						...
						?>
								...
								<p></p><hr><p></p>
								<div class="file-option">
									<a href="<?= $_CHAT_FILE ?>?room_id=<?= $room["room_id"] ?>"><button class="btn btn-success btn-rounded btn-sm"><i class="fa fa-sign-in"></i> Ingresar directo</button></a>
								</div>
										</div>
								</div>
							<?php endwhile;
							if(mysqli_num_rows($query) < 1){
								echo "¡Aún no te has unido a ningun!";
							}
							
							$query1 = mysqli_query($db, "select * from room_member where user_id = {$user["id"]}") or error("No puedo obtener los datos del grupo", $_HOME_FILE);
...
function delete_room(room_id){
	var c = confirm("¿Está seguro? ¡Presiona OK si quieres eliminar esta sala!");
	if (c == true) {
		$.ajax({
