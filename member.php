                    ...
                    <div class="ibox float-e-margins">
						...
						?>
								...
								<div class="social-avatar">
									<small class="text-muted">Se unio en fecha <?= $req["join_date"] ?></small>
								</div>
								<div class="social-body">
									<strong><?= $_user["firstName"] . " " . $_user["lastName"]; ?></strong> actualmente es miembro del grupo.
								<p></p><hr><p></p>
								<div class="file-option">
									...
									<button class="btn btn-danger btn-rounded btn-sm" onclick="delete_member(<?= $req["user_id"] ?>,<?= $req["room_id"] ?>)"><i class="fa fa-times"></i> Quitar miembro</button>
								</div>
										</div>
								</div>
							<?php 
							...
							if(mysqli_num_rows($query) < 1){
								echo "La sala aún no tiene miembros!";
							}
							?>
                        </div>
                    </div>
...
function approve_request(user_id, room_id){
	$.ajax({
		...
		success: function(r) {
			if(r.success){
				$("#request-user-" + user_id).remove()
				toastr.success("No se puede aprobar este usuario")
			} else {
				toastr.error("No se puede aprobar este usuario")
			}
		},
		error: function(){
