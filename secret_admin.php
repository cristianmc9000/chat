                    ...
                    <div class="ibox float-e-margins">
						...
						?>
								...
								<div class="social-body">
									<strong>Nombre: </strong> <?= !empty($room["room_description"]) ? $room["room_description"] : "null"; ?>
								<p></p><hr><p></p>
								<div class="file-option">
									<button class="btn btn-danger btn-rounded btn-sm" onclick="delete_room(<?= $room["room_id"] ?>)"><i class="fa fa-times"></i> Borrar Grupo</button>
								</div>
										</div>
								</div>
							<?php endwhile;
							...
							<li class="<?php echo ($_GET['room_page']-1 != 0) ? 'prev' : 'prev disabled';?>"><a href="secret_admin.php?room_page=<?php echo ($_GET['room_page']-1 != 0) ? $_GET['room_page']-1 : 1;?>">Atras</a></li>
							<?php for($i = 1; $i <= $n; $i++): ?>
				...
				<div id="user_control" class="col-lg-12">
                    <div class="ibox float-e-margins">
						...
						?>
								...
								<p></p><hr><p></p>
								<div class="file-option">
									<button class="btn btn-danger btn-rounded btn-sm" onclick="delete_user(<?= $_user["id"] ?>)"><i class="fa fa-times"></i> Borrar Usuarios</button> 
									<button id="ban-user-id-<?= $_user["id"] ?>" <?php if(mysqli_num_rows($ban_check) > 0) echo 'style="display: none;"'; ?> class="btn btn-warning btn-rounded btn-sm" onclick="ban_user(<?= $_user["id"] ?>)"><i class="fa fa-ban"></i> Prohibir usuario</button>
									
									<button id="unban-user-id-<?= $_user["id"] ?>" <?php if(mysqli_num_rows($ban_check) < 1) echo 'style="display: none;"'; ?> class="btn btn-warning btn-rounded btn-sm" onclick="unban_user(<?= $_user["id"] ?>)"><i class="fa fa-ban"></i> Usuario no baneado</button> 
								
									<button id="promote-admin-id-<?= $_user["id"] ?>" class="btn btn-info btn-rounded btn-sm" <?php if($_user["admin"] == 1) echo 'style="display: none;"'; ?>onclick="promote_user(<?= $_user["id"] ?>)"><i class="fa fa-level-up"></i> Ascender a administrador</button>
									
									<button id="unpromote-admin-id-<?= $_user["id"] ?>" class="btn btn-info btn-rounded btn-sm" <?php if($_user["admin"] == 0) echo 'style="display: none;"'; ?> onclick="unpromote_user(<?= $_user["id"] ?>)"><i class="fa fa-level-up"></i> Eliminar administrador</button>
								</div>
										</div>
								</div>
							<?php endwhile;
...
function delete_user(user_id){
	var c = confirm("¿Está seguro? Presiona OK si quieres eliminar este usuario!");
	if (c == true) {
		$.ajax({
...
function ban_user(user_id){
	var reason;
	var input = prompt("Razón por la que baneas a este usuario:", "Violación de reglas");
	reason = input;
...
function unban_user(user_id){
	var c = confirm("¿Está seguro? Presiona OK si quieres hacer esto");
...
function promote_user(user_id){
	var c = confirm("¿Está seguro? Presiona OK si quieres hacer estos");
